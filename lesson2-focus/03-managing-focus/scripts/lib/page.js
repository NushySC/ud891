					...
					(function (process) {
						...
						function page(path, fn) {
							...
							if ('function' === typeof fn) {
								...
								for (var i = 1; i < arguments.length; ++i) {
									page.callbacks.push(
										route.middleware(arguments[i])
									);
								}
								// show <path> with [state]
							} else if ('string' === typeof path) {
								page[
									'string' === typeof fn ? 'redirect' : 'show'
								](path, fn);
								// start [options]
							} else {
								page.start(path);
						...
						function Context(path, state) {
							...
							if (!hashbang) {
								...
								this.path = parts[0];
								this.hash =
									decodeURLEncodedURIComponent(parts[1]) ||
									'';
				...
				function (require, module, exports) {
					...
					function arrayToRegexp(path, keys, options) {
						...
						for (var i = 0; i < path.length; i++) {
							parts.push(
								pathToRegexp(path[i], keys, options).source
							);
						}
